...
const response_update_password_dto_1 = require("./dto/response.update.password.dto");
function validatePassword(isPasswordValid) {
    if (!isPasswordValid) {
        throw new common_1.BadRequestException('Invalid password');
    }
}
function buildTokenDto(token) {
    const tokenDto = new response_token_dto_1.ResponseTokenDto();
    tokenDto.message = 'Generated token ok';
    tokenDto.token = token;
    return tokenDto;
}
function buildUserPyload(user) {
    const userPayload = {
        sub: user.id,
        id: user.id,
        email: user.email,
...
function buildResponseUpdatePasswordDto() {
    const responseUpdatePasswordDto = new response_update_password_dto_1.ResponseUpdatePasswordDto();
    responseUpdatePasswordDto.message = 'Password changed successfully';
    return responseUpdatePasswordDto;
}
function buildUpdateUserDto(password) {
